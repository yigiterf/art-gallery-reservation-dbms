...
const SellerDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'yayinla' | 'eserlerim' | 'etkinlik' | 'analiz' | 'destek'>('yayinla');
  
  // Eser States
  const [form, setForm] = useState({ baslik: '', aciklama: '', fiyat: '' });
  const [imageFile, setImageFile] = useState<File | null>(null);
  const [imagePreview, setImagePreview] = useState('');
  const [myArtworks, setMyArtworks] = useState<any[]>([]);
  
  // Etkinlik States
  const [etkinlikForm, setEtkinlikForm] = useState({ baslik: '', tarih_saat: '', ucret: '', kontenjan: '' });
  ...
  const handleEtkinlik = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setEtkinlikForm({ baslik: '', tarih_saat: '', ucret: '', kontenjan: '' });
      setTimeout(() => setSuccess(''), 3000);
    } catch (err) {
              ...
              <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-200">
                <form onSubmit={handlePublishEser} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      <label className="block text-sm font-semibold text-slate-700 mb-2">Görsel Yükle Ekla</label>
                ...
                <form onSubmit={handleEtkinlik} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                      ...
                      <input type="datetime-local" required value={etkinlikForm.tarih_saat} onChange={e => setEtkinlikForm({...etkinlikForm, tarih_saat: e.target.value})} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-500 transition-all outline-none" />
                    </div>
                    <div>
